...
namespace detail {
    ...
    static size_t consume(char (&result)[N], std::span<uint8_t> data) noexcept {
        ...
        // result[N] is one past the end of a char[N] - an out-of-bounds write. This overload
        // is not instantiated anywhere today, so this is disarming a trap rather than fixing a
        // live bug, but it would be a silent buffer overrun the first time anyone used it.
        if constexpr (N > 0) result[N - 1] = '\0';
        return N;
    }
