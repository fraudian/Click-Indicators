...
public:
    ...
    [[nodiscard]] static Result<Self> importData(std::span<uint8_t> data) {
        ...
        size_t sizes;
        stream >> sizes;
        // The count comes out of the file, so it is attacker-controlled. Every death costs at
        // least one byte on the wire, so a count larger than the bytes remaining cannot be
        // honest - and reserve() would commit that much memory before a single one is read.
        r.deaths.reserve(std::min<size_t>(sizes, stream.size()));
        ...
        stream >> sizes;
        r.inputs.reserve(std::min<size_t>(sizes, stream.size()));
        ...
        p = 0;
        while (!stream.empty()) {
            // binary_reader consumes NOTHING when the stream is too short to hold the value:
            // detail::consume returns 0 and subspan(0) is a no-op, so empty() never becomes
            // true. Without this, a truncated or crafted file spins here forever, appending an
            // input every iteration until the process has committed all available memory. A
            // pass that reads no bytes is the end of the data, however the file claims it ends.
            const size_t bytesLeftAtPassStart = stream.size();
            InputType input;
            uint64_t packed;
            stream >> packed;
            ...
            // Nothing was consumed this pass, so nothing ever will be. See the note above.
            if (stream.size() == bytesLeftAtPassStart) break;
        }
